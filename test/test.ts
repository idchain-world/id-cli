...
test("--version: shows version", () => {
  ...
  assertIncludes(output, "0.");
});

// ─── Mint USDC ───────────────────────────────────────────────────────────────

test("mint-usdc: mint 1000 USDC to test wallet", () => {
  const output = run("mint-usdc --amount 1000");
...
test("info: look up existing name (agent-0)", () => {
  const output = run("info agent-0");
  assertIncludes(output, "agent-0.xid.eth");
  assertIncludes(output, "Owner:");
  assertIncludes(output, "Node:");
});

test("info: full domain path works", () => {
  const output = run("info agent-0.xid.eth");
  assertIncludes(output, "agent-0.xid.eth");
  assertIncludes(output, "Owner:");
});

test("info: nonexistent name shows not registered", () => {
  const output = run("info nonexistent-name-xyz123");
  assertIncludes(output, "not registered");
});

// ─── Explore ─────────────────────────────────────────────────────────────────

test("explore: lists names", () => {
...
test("register: register with text records", () => {
  const output = run(
    'register --text description="text-record-test"'
  );
  assertIncludes(output, "Registered");
});
...
test("register: register with --address flag", () => {
  const output = run(
    `register --address ${ANVIL_ACCOUNT_0}`
  );
  assertIncludes(output, "Registered");
});
...
test("info: verify newly registered name exists", () => {
  const output = run(`info ${registeredLabel}`);
...
test("records: check records of new name", () => {
  const output = run(`records ${registeredLabel}`);
  assertIncludes(output, registeredLabel);
});
...
test("set-text: set description on new name", () => {
  const output = run(
    `set-text ${registeredLabel} description "test agent"`
  );
...
test("set-text: set agent-context record", () => {
  const output = run(
    `set-text ${registeredLabel} agent-context '{"type":"agent"}'`
  );
  assertIncludes(output, "Set");
...
test("set-addr: set ETH address on new name", () => {
  const output = run(
    `set-addr ${registeredLabel} ${ANVIL_ACCOUNT_0}`
  );
...
test("set-addr: set multi-coin address (coin type 0 = BTC)", () => {
  ...
  const output = run(
    `set-addr ${registeredLabel} ${btcAddrHex} --coin-type 0`
  );
...
test("set-contenthash: set content hash on new name", () => {
  ...
  const output = run(
    `set-contenthash ${registeredLabel} ${contentHash}`
  );
...
test("create-subname: create test-sub under registered name", () => {
  const output = run(
    `create-subname test-sub --parent ${registeredLabel}`
  );
...
test("create-subname: create with custom owner", () => {
  const output = run(
    `create-subname owned-sub --parent ${registeredLabel} --owner ${ANVIL_ACCOUNT_1}`
  );
  assertIncludes(output, "Created");
...
test("create-subname: using full domain path for parent", () => {
  const output = run(
    `create-subname path-sub --parent ${registeredLabel}.xid.eth`
  );
  assertIncludes(output, "Created");
  assertIncludes(output, `path-sub.${registeredLabel}.xid.eth`);
});

// ─── Info on subname ─────────────────────────────────────────────────────────

test("info: verify subname exists", () => {
  const output = run(`info test-sub.${registeredLabel}`);
  assertIncludes(output, `test-sub.${registeredLabel}`);
  assertIncludes(output, "Owner:");
});

test("info: verify subname with custom owner", () => {
  const output = run(`info owned-sub.${registeredLabel}`);
  assertIncludes(output, ANVIL_ACCOUNT_1);
});
...
test("set-text: set text record on subname", () => {
  const output = run(
    `set-text test-sub.${registeredLabel} role "sub-agent"`
  );
  assertIncludes(output, "Set");
...
test("list-subnames: list subnames (may need indexer)", () => {
  const output = run(`list-subnames ${registeredLabel}`);
  assert(
...
test(
  ...
  () => {
    const output = run(
      `register-agent ${registeredLabel}`
    );
    assertIncludes(output, "Registered");
...
test(
  ...
  () => {
    ...
    const output = run(
      `link-agent ${registeredLabel} ${agentId}`
    );
...
test("transfer: transfer name to anvil account 1", () => {
  const output = run(
    `transfer ${registeredLabel} --to ${ANVIL_ACCOUNT_1}`
  );
...
test("transfer: fails when not owner", () => {
  const output = run(
    `transfer ${registeredLabel} --to ${ANVIL_ACCOUNT_0}`,
    true
  );
...
test("dry-run: transfer shows proposal without executing", () => {
  const output = run(
    `transfer ${registeredLabel} --to ${ANVIL_ACCOUNT_0} --dry-run`
  );
...
test("dry-run: set-text shows proposal", () => {
  const output = run(
    `set-text ${registeredLabel} description "test" --dry-run`
  );
  assertIncludes(output, "Transaction Proposal");
...
test("dry-run: create-subname shows proposal", () => {
  const output = run(
    `create-subname dry-sub --parent ${registeredLabel} --dry-run`
  );
  assertIncludes(output, "Transaction Proposal");
